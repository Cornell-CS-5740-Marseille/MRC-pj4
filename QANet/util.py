...
def get_batch_data():
    ...
    x_q_c = np.random.randint(low=0, high=param.char_vocab_size,
                              size=(param.num_samples, param.max_question_words, param.max_chars))
    # outputs
...
def positional_encoding(inputs, scope="positional_enc", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        pos_enc_matrix = np.array(
            [[pos / np.power(10000, 2. * i / dims) for i in range(dims)] for pos in range(max_length)])
...
def encoder_block(inputs, num_conv_layer=4, filters=128, kernel_size=7, num_att_head=8, scope="encoder_block",
                  reuse=None):
    '''
